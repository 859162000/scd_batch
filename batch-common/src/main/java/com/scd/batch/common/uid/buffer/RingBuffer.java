...
import com.scd.batch.common.utils.concurrent.PaddedAtomicLong;
...
public class RingBuffer {
    ...
    public static final int DEFUALT_PADDING_PERCENT = 50;
    ...
    /**
     * Constructor with buffer size, paddingFactor default as {@value #DEFUALT_PADDING_PERCENT}
     * 
     * @param bufferSize must be positive & a power of 2
     */
    public RingBuffer(int bufferSize) {
        this(bufferSize, DEFUALT_PADDING_PERCENT);
    }
    ...
    public RingBuffer(int bufferSize, int paddingFactor) {
        ...
        Preconditions.checkState(bufferSize > 0L, "RingBuffer size must be positive");
        Preconditions.checkState(Integer.bitCount(bufferSize) == 1, "RingBuffer size must be a power of 2");
        Preconditions.checkState(paddingFactor > 0 && paddingFactor < 100, "RingBuffer size must be positive");
    ...
    public synchronized boolean put(long uid) {
        ...
        int nextTailIndex = calSoltIndex(currentTail + 1);
    ...
    public long take() {
        ...
        Preconditions.checkState(nextCursor >= currentCursor, "Curosr can't move back");
        ...
        // cursor catched the tail, means that there is no more available UID to take
        if (nextCursor == currentCursor) {
            rejectedTakeHandler.rejectTakeBuffer(this);
        }

        // 1. check next slot flag is CAN_TAKE_FLAG
        int nextCursorIndex = calSoltIndex(nextCursor);
        Preconditions.checkState(flags[nextCursorIndex].get() == CAN_TAKE_FLAG, "Curosr not in can take status");
     ...
     */
    protected int calSoltIndex(long sequence) {
