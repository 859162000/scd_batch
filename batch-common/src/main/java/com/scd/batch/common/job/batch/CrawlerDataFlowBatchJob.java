...
public abstract class CrawlerDataFlowBatchJob extends AbstractExecutor {
    ...
    public void execute(ExecutorContext context) {
        try {
            ...
            while (CollectionUtils.isNotEmpty(sourceLines)) {
                ...
                if (!forwardSourceCursor(context)) {
                    break;
                }
     ...
     */
    private boolean forwardSourceCursor(ExecutorContext context) {
        Pagination pagination = context.getAttach(Pagination.class);

        if (pagination.getCurPage() < pagination.getPageCount()) {
