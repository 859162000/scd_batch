 ...
 * 充值对账任务
 */
public class SaveCalculatorJob extends ReconciliationBatchJob {
    ...
    public void reconciliaiton(SourceType sourceType,
        ...
        if (sourceType.getType() == SourceType.HUIFU.getType()) {
            ...
            List<SaveTransferEntity> transferEntityList = CommonUtil.transform2Bean(sourceLines,
                    SaveTransferEntity.class);

            for (SaveTransferEntity transfer : transferEntityList) {

                // TODO 先使用HASH，考虑使用redis，不存在，则写入hash
                transferRepo.computeIfAbsent(transfer.getOrdId(),
                        new Function<String, SaveErrorBase>() {
                            public SaveErrorBase apply(String t) {
                                return new SaveErrorBase(t, null, transfer, TransferErrorType.HUIFU);
                            }
                        });
                ...
                transferRepo.computeIfPresent(transfer.getOrdId(),
                        new BiFunction<String, TransferErrorBase, TransferErrorBase>() {
                            ...
                            public TransferErrorBase apply(String t, TransferErrorBase u) {
                                ...
                                SaveErrorBase cashErrorEntity = (SaveErrorBase) u;
            ...
            for (WithdrawL withdrawL : withdrawLList) {
                ...
                transferRepo.computeIfAbsent(withdrawL.getOrderId(),
                        new Function<String, SaveErrorBase>() {
                            public SaveErrorBase apply(String t) {
                                return new SaveErrorBase(t, withdrawL, null, TransferErrorType.SCD);
                            }
                        });
    ...
    private void batchInsertResult2DB(ConcurrentHashMap<String, TransferErrorBase> transferRepo) {
        ...
        transferRepo.forEach((k, v) -> {
            ...
            SaveErrorBase saveErrorBase = (SaveErrorBase) v;

            // 差错处理状态为初始态
            TransferErrorEntity entity = new TransferErrorEntity(k,
                    TransferType.RECHARGE.getType(),
                    saveErrorBase.getTransferErrorType().getType(),
                    TransferErrorStatus.INIT.getType()
            );

            if (saveErrorBase.getWithdrawL() != null) {
                entity.setaValue(saveErrorBase.getWithdrawL().getValue());
                entity.setaJson(JsonUtils.toJson(saveErrorBase.getWithdrawL()));
            }

            if (saveErrorBase.getTransferEntity() != null) {
                entity.setbValue(saveErrorBase.getTransferEntity().getValue());
                entity.setbJson(JsonUtils.toJson(saveErrorBase.getTransferEntity()));
            }
